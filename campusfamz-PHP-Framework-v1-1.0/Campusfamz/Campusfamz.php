...
function send_shout($param){
	...
	$response = $feed->send_shout($user_id, $shout);
	exit(json_encode($response));
}
...
function view_shout($param1, $ext){
	$feed = $this->registry->getObject('feed_class');
	$auth = $this->registry->getObject('auth_class');
	
	$param1 = $auth->filter_data($param1);
	$param2 = $auth->filter_data($ext);
...
function get_photos($param1, $ext, $page){
	$photo = $this->registry->getObject('photo_class');
	$auth = $this->registry->getObject('auth_class');
	
	$user_id = $auth->filter_data($param1);
	$folder = $auth->filter_data($ext);
	$page = $auth->filter_data($page);
	
	$response = $photo->get_photos($folder, $user_id, $page);
	exit(json_encode($response));
}
...
function view_photo($param, $ext){
	...
	$response = $photo->view_photo($folder, $photo_id);
	exit(json_encode($response));
}
...
function comment_photo($param){
	...
	$response = $photo->comment_photo($user_id, $image_id, $comment);
	exit(json_encode($response));
}
...
function photo_nav($param){
	$photo = $this->registry->getObject('photo_class');
	$auth = $this->registry->getObject('auth_class');
	
	$image_id = $auth->filter_data($param);

	$response = $photo->photo_nav($image_id);
	exit(json_encode($response));
...
function send_message($param){
	...
	$user_id = $auth->filter_data($_POST['user_id']);
	$message_to = $auth->filter_data($_POST['message_to']);
...
function read_message($param, $ext){
	$message = $this->registry->getObject('message_class');
	$auth = $this->registry->getObject('auth_class');
	
	$user_id = $auth->filter_data($param);
	$message_id = $auth->filter_data($ext);

	$response = $message->read_message($user_id, $message_id);
	exit(json_encode($response));
...
function topic_details($param){
	$forum = $this->registry->getObject('forum_class');
	$auth = $this->registry->getObject('auth_class');
	
	$topic_id = $auth->filter_data($param);
	
	$response = $forum->topic_details($topic_id);
	exit(json_encode($response));
}
...
function post_comments($param, $ext, $page){
	$forum = $this->registry->getObject('forum_class');
	$auth = $this->registry->getObject('auth_class');
...
function forum_cat_name($param){
	$forum = $this->registry->getObject('forum_class');
	$auth = $this->registry->getObject('auth_class');
	
	$cat_id = $auth->filter_data($param);
	$response = $forum->forum_cat_name($cat_id);
	exit(json_encode($response));
}
...
function registerDev($param){
	...
	$user_id = $auth->filter_data($_POST['user']);
